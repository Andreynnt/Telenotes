...
void DBConnector::checkConnection(){//checking the connection. If smth is wrong trying to reconnect.
    if(!connector.isOpen()){
        if(!connectDB()){
...
int main() {
    ...
    QSqlDatabase conn1, conn2;
    DBConnector connection1(conn1,12345);
    connection1.connectDB();
    connection1.authorizeUser();

    connection1.insertQuery("Thats my secondary","My TEXT and not my text","testtag1, testtag2");
    QString out;
    out = QString::fromUtf8(connection1.selectAllQuery().c_str());
    qDebug()<<out;

    DBConnector connection2(conn2,22345);
    connection2.connectDB();
    connection2.authorizeUser();
    connection2.insertQuery("Thats my secondary","My TEXT and not my text","testtag1, testtag2");

    out = QString::fromUtf8(connection2.selectAllQuery().c_str());
    qDebug()<<out;
    //selectByNameQuery(db,12345,"Thats my first");
    //selectAllQuery(db, 11011995);
    return 0;
}
